...
contract EntryPoint is IEntryPoint, StakeManager{
     using UserOperationLibrary for UserOperation;
     struct MemoryUserOp {
        address sender;
        uint256 nonce;
    ...
    // handle user operations of accounts that don’t require any signature aggregator
    function handleOps(UserOperation[] calldata ops, address payable beneficiary)public{
        uint256 ops_length = ops.length;
        UserOpInfo[] memory opInfos = new UserOpInfo[](ops_length);
         unchecked {
        for (uint256 i = 0; i < ops_length; i++) {
